-- =========================================================
-- Redefine finalize_report_generation to fix lock ordering
-- (project -> report, matching claim_report_generation).
-- Also add per-section invariant: every section must be
-- complete or failed before we finalize.
-- =========================================================
...
BEGIN
  -- Non-locking read to discover project_id
  SELECT project_id INTO v_project_id FROM public.reports WHERE id = p_report_id;
  IF NOT FOUND THEN
    RETURN jsonb_build_object('finalized', false, 'error', 'not_found');
  END IF;

  -- Lock in consistent order: project first, then report
  ...
  -- Attempt ownership FIRST — so a different attempt is always superseded,
  -- even if a prior attempt already finalized this row.
  ...
  -- Same attempt, already finalized -> idempotent success
  ...
  -- Invariant: every canonical section must be complete or failed
  FOREACH v_section IN ARRAY v_pending_keys LOOP
    v_status_val := p_generation_status ->> v_section;
    IF v_status_val IS NULL OR v_status_val NOT IN ('complete','failed') THEN
      RETURN jsonb_build_object(
        'finalized',    false,
        'not_ready',    true,
        'section',      v_section,
        'section_state', COALESCE(v_status_val, 'missing'),
        'report_id',    v_report.id,
        'project_id',   v_report.project_id
      );
    END IF;
...
-- =========================================================
-- Redefine claim_report_generation so every ordinary existing-report
-- resume sets resumed=true (including legacy uncharged resume).
-- Charging status is independent from resumed status.
-- =========================================================
...
BEGIN
  ...
  ELSE
    -- Ordinary resume: preserve complete sections, always set resumed=true.
    ...
    IF v_report.credits_charged_at IS NULL THEN
      -- Legacy uncharged: charge once, still a resume.
      IF v_profile.ai_credits_used + v_credits > v_profile.ai_credits_monthly THEN
        RAISE EXCEPTION 'insufficient credits' USING ERRCODE = 'P0001';
      END IF;
      UPDATE public.profiles
